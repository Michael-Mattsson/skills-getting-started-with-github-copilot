    ...
    try {
      ...
      const participantsHtml = participants.length
        ? `<ul class="participant-list">${participants
            .map(
              (participant) =>
                `<li class="participant-item"><span class="participant-email">${participant}</span><button class="participant-remove" type="button" data-activity="${encodeURIComponent(
                  name
                )}" data-email="${participant}" aria-label="Remove participant ${participant}">&times;</button></li>`
            )
            .join("")}</ul>`
